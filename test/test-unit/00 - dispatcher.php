<?php
    ...
    pirogue_test_execute(
        'pirogue_dispatcher_create_url: empty',
        fn() => pirogue_dispatcher_create_url('', []) == $GLOBALS['.pirogue.dispatcher.address']
            ? []
    ...
    pirogue_test_execute('pirogue_dispatcher_create_url: path and data', function () {
        ...
        return $url == "{$GLOBALS['.pirogue.dispatcher.address']}/{$path}?{$data_label}={$data_value}"
            ? []
            :  [ "Invalid url '{$url}' returned."];
    });
