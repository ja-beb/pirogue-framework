...
function cdn_test(array $server_list, int $count): array
{
    ...
    for ($i = 0; $i < $count; $i++) {
        ...
        if (
            ($GLOBALS['._pirogue.cdn.current_index'] - $_start_index) != 1
            && 0 != $GLOBALS['._pirogue.cdn.current_index']
        ) {
            array_push(
                $_error_list,
                sprintf(
                    'pirogue_cdn_create_url(): Index not properly incremented. %d, %d',
                    $_start_index,
    ...
    // Run test
    pirogue_test_execute('pirogue_cdn_init: uri = 0, pirogue_cdn_create_url x3', function () {
        try {
            cdn_test([], 3);
    ...
    pirogue_test_execute(
        'pirogue_cdn_init: uri = 1, pirogue_cdn_create_url x4',
        fn() => cdn_test(['cdn.localhost'], 4)
    );

    pirogue_test_execute(
        'pirogue_cdn_init: uri = 4, pirogue_cdn_create_url x12',
        fn() => cdn_test([
            'cdn.00.localhost',
            'cdn.01.localhost',
            'cdn.02.localhost',
