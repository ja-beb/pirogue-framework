...
/**
 * Testing pirogue\dispatcher_route_register()
 * php version 8.0.0
 *
...
pirogue_test_execute('pirogue\dispatcher_route_register()', function () {
    $route = dispatcher_route_create('namespace', 'action', 'method');
    $index = dispatcher_route_register($route);

    $regsitered_route = $GLOBALS['._pirogue.dispatcher.call_stack'][$index - 1] ?? null;
