 ...
 * @internal
 * @var string $GLOBALS['._pirogue.controller_namespace']
 */
$GLOBALS['._pirogue.controller_namespace'] = '';

/**
 * initialize library.
 * @internal
 * @uses $GLOBALS['._pirogue.controller_namespace']
 * @param string $name the name of the controller that is loaded.
 * @return void.
 */
function _init(string $name) : void
{
    $GLOBALS['._pirogue.controller_namespace'] = $name;
}
...
{
    unset($GLOBALS['._pirogue.controller_namespace']);
}
...
function has_access(?int $user_id) : bool
{
    return call_user_func(
        sprintf('%s\has_access', $GLOBALS['._pirogue.controller_namespace']),
        $user_id
    );
}
...
{
    return call_user_func(
        sprintf('%s\%s', $GLOBALS['._pirogue.controller_namespace'], $action),
        $request_path,
        $request_data,
        $form_data
    );
